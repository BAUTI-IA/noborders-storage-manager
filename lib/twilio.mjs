...
export function verifyTwilioSignature(authToken, signatureHeader, url, params) {
  if (!authToken) return false; // not configured → nothing can be verified: refuse (never fail open)
...
// Outbound WhatsApp message via the Twilio REST API. Used instead of a TwiML
// reply because Twilio cuts webhooks off at ~15s and the AI extraction can take
// longer — the webhook ACKs immediately and this delivers the answer after.
export async function sendWhatsApp(to, body) {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  const token = process.env.TWILIO_AUTH_TOKEN;
  const from = process.env.TWILIO_WHATSAPP_FROM; // e.g. "whatsapp:+14155238886" (sandbox)
  if (!sid || !token || !from) throw new Error("Faltan TWILIO_ACCOUNT_SID / TWILIO_AUTH_TOKEN / TWILIO_WHATSAPP_FROM");
