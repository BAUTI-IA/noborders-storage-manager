...
const ASK_TOOL = {
  ...
  name: "crm_ask",
  description:
    "Hand a question to the CRM's text agent when a single SELECT won't do it: multi-step analysis, business math like a job's real debt, settlements, anything about how the company works, or how the CRM itself is used (it can read the user guide). It is noticeably slower than crm_lookup, so only use it when crm_lookup can't answer.",
  parameters: {
    type: "object",
