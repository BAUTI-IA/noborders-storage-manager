...
import { checkSqlAccess, normalizeSql, writesEnabled } from "./agentWrite.mjs";
...
const VOICE_T = {
  es: {
    ...
    slow: "Esa consulta tardó demasiado.",
...
export async function buildVoiceInstructions({ actor, canWrite, langLock }) {
  ...
  return [
    ...
    "",
    "HOW TO SPEAK:",
    "- English is your DEFAULT. Argentine Spanish is first class too, and the team mixes them: speak Spanish only while the person is speaking Spanish, and switch the moment they switch, even mid-conversation. When you can't tell, speak English. Never ask which language they want — just follow them.",
    ...
    // Nobody has spoken yet, so there is no language to mirror: the greeting
    // goes in English, the default, unless the person locked the other one.
    langLocked(langLock)
      ...
      : "- Open the conversation with ONE short greeting in English (the default), then wait. After that, mirror whatever they actually speak: English unless they speak to you in Spanish.",
  ].join("\n");
}
...
export async function buildVoiceSession({ actor, canWrite, transport, lang, langLock }) {
  return {
    type: "realtime",
    model: VOICE_MODEL,
    instructions: await buildVoiceInstructions({ actor, canWrite, langLock }),
...
export async function runVoiceTool({ name, input, convoKey, actor, lang }) {
  ...
  try {
    switch (name) {
      case "crm_lookup": {
        const q = normalizeSql(input?.query);
