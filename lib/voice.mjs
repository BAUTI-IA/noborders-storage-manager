...
export async function buildVoiceInstructions({ actor, canWrite, lang }) {
  ...
  return {
    input: {
      ...
      transcription: {
        ...
        ...(VOICE_TRANSCRIBE_LANGUAGE ? { language: VOICE_TRANSCRIBE_LANGUAGE } : {}),
      },
...
export async function buildVoiceSession({ actor, canWrite, transport, lang }) {
  return {
    type: "realtime",
    model: VOICE_MODEL,
    instructions: await buildVoiceInstructions({ actor, canWrite, lang }),
...
export async function mintVoiceSession({ actor, canWrite, transport = "webrtc", lang }) {
  const key = process.env.OPENAI_API_KEY;
  if (!key) throw new Error(vt(lang).noKey);
  const t = transport === "websocket" ? "websocket" : "webrtc";
  const session = await buildVoiceSession({ actor, canWrite, transport: t, lang });
