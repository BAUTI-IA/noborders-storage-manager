...
import {
  mergePipelineSettings, clampRawText, isAllowedSender, senderAddress, holdDates,
  leadsNeedingAttention, isEvaluable, num, nearestTruck, LEAD_FIELDS,
  computeActuals, jobRunWindow, withinRange,
  deadheadOrigin, DEADHEAD_ORIGINS,
} from "../src/pipelineData.js";
import { reverseZip } from "./geo.mjs";
...
/**
 * @param approachZip where the OUT leg starts. Defaults to baseZip; pass the
 *   truck's live position to measure the approach from where it really is.
 *   The BACK leg always aims at baseZip — the truck eventually comes home.
 */
export async function milesFor({ originZip, destZip, baseZip, approachZip, token, appUrl }) {
  need();
  const o = zip5(originZip), d = zip5(destZip), b = zip5(baseZip);
  const a = zip5(approachZip) || b;
  if (!/^\d{5}$/.test(o) || !/^\d{5}$/.test(d)) return null;

  let loaded = await cachedMiles(o, d);
  let out = a ? await cachedMiles(a, o) : 0;
  let back = b ? await cachedMiles(d, b) : 0;

  // The caller's own origin first: it is the deployment that is answering right
  // now, so it works without APP_URL configured (which silently left every
  // uncached route unmeasured) and never points at a stale domain.
  const base = String(appUrl || process.env.APP_URL || "").replace(/\/+$/, "");
  if (loaded == null && token && base) {
    try {
      const qs = new URLSearchParams({ origin: o, dest: d, ...(a ? { base: a } : {}) });
      const res = await fetch(`${base}/api/distance?${qs}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) console.error("[pipeline] /api/distance", res.status, (await res.text()).slice(0, 300));
      if (res.ok) {
    ...
    } catch (e) { console.error("[pipeline] /api/distance:", e?.message || e); }
  }
  ...
  return {
    ...
    deadheadKnown: a ? (out != null && back != null) : false,
  };
}
...
export async function nearestTruckTo(originZip) {
  ...
  return {
    ...
    lat: Number(best.truck.last_lat),
    lng: Number(best.truck.last_lng),
  };
}
 ...
 */
export async function evaluateLead(lead, { token = null, appUrl = null } = {}) {
  need();
  ...
  // The empty miles that matter are the ones the truck that would actually take
  // this job has to drive to reach the pickup — not the ones from a yard it is
  // nowhere near. The ELD gives lat/lng and the cost model speaks ZIPs, so the
  // position is reverse-geocoded (cached forever) and used as the approach.
  // A failed lookup is not an error: it just falls back to the base ZIP.
  let truckZip = "";
  if (near && Number.isFinite(near.lat) && Number.isFinite(near.lng)) {
    truckZip = (await reverseZip(near.lat, near.lng)) || "";
  }
  const origin = deadheadOrigin(truckZip, settings.baseZip);

  const miles = await milesFor({
    originZip: lead.origin_zip, destZip: lead.dest_zip,
    baseZip: settings.baseZip, approachZip: origin.zip, token, appUrl,
  });
  ...
  const patch = {
    ...
    parsed: {
      ...(lead.parsed || {}),
      nearest_truck: near ? { ...near, zip: truckZip || null } : null,
      // Which end the empty miles were measured from, so the board can say so
      // instead of leaving the operator to assume.
      deadhead_from: origin.from,
      deadhead_zip: origin.zip || null,
    },
  };
  const { error: upErr } = await admin.from("job_leads").update(patch).eq("id", lead.id);
  if (upErr) return { skipped: "db", error: upErr };

  return { evaluation: ev, result: r, nearest: near, deadheadFrom: origin.from };
}
...
const EXTRACT_SCHEMA = {
  ...
  properties: {
    is_job_offer: {
      type: "boolean",
      description: "true only when the message offers, quotes or asks us to haul a specific move/shipment; false for newsletters, marketing, invoices, account notifications and anything else",
    },
  ...
  required: ["is_job_offer", "broker_name", "broker_job_number", "customer", "origin_city", "origin_state", "origin_zip",
...
function extractPrompt({ text, brokers, today }) {
  return [
    ...
    "- is_job_offer is false for anything that is not a specific move (newsletters, product marketing, receipts, security or account notices). When false, leave every other field empty.",
    "",
    `TODAY: ${today}`,
...
/** Read free text into lead fields. Returns { fields, lowConfidence, brokerName, isJobOffer }. */
export async function extractLeadFields(text, { brokers = [], today = todayISO() } = {}) {
  ...
  // A missing flag reads as a job: only an explicit false drops a message.
  return { fields, lowConfidence: low, brokerName: s(p.broker_name), isJobOffer: p.is_job_offer !== false };
}
 ...
 *
 * `requireJob` (inbound email) returns null instead of saving when the model
 * says the message is not a job offer. A pasted lead never passes it: a person
 * pasting text has already decided it is a job.
 */
export async function createLeadFromText({ text, source = "manual", sourceRef = null, createdBy = null, sender = null, requireJob = false }) {
  need();
  const names = await brokerNames();
  const { fields, lowConfidence, brokerName, isJobOffer } = await extractLeadFields(text, { brokers: names });
  if (requireJob && !isJobOffer) return null;
  const broker = await matchBroker(brokerName);
  const row = {
    source, source_ref: sourceRef, created_by: createdBy,
    raw_text: clampRawText(text),
    // `sender` goes in parsed rather than its own column so the per-sender rate
    // limit needs no migration — and so the board can show who sent a lead.
    parsed: {
      confidence: Object.fromEntries(lowConfidence.map((f) => [f, "low"])),
...
/**
 * Record a dropped email where a person can see it.
 *
 * The webhook answers a flat 202 whatever happens, so that a stranger probing
 * it cannot learn which domains we accept — which also means a legitimate
 * broker who is not on the allowlist yet would vanish without a trace. This is
 * that trace: an action_log row, which the Pipeline board counts for today and
 * the Trash → History tab lists in full.
 *
 * `from` and `subject` are attacker-controlled, so they are clamped and stored
 * as data; nothing here is ever executed or fed back to a model. Audit failure
 * never blocks ingestion, exactly as in lib/agentWrite.mjs.
 */
async function logDrop({ reason, from, subject }) {
  ...
  try {
    const { error } = await admin.from("action_log").insert({
      ...
      entity_id: "*",
      action: "dropped",
      label: `${reason} — ${cut(from, 200) || "(no sender)"}`,
      after: { reason, from: cut(from, 200), subject: cut(subject, 300) },
      user_email: "inbound-email",
    });
    if (error) console.error("[pipeline] logging a dropped email:", error.message);
  } catch (e) {
    console.error("[pipeline] logging a dropped email:", e?.message || e);
  }
}

/**
 * The Gmail Apps Script / Cloudflare Worker posts a raw email here. Every gate
 * fails closed: an unconfigured allowlist accepts nobody (unless an admin turned
 * on acceptAllSenders), a message that is not a job offer is dropped, and a lead is only ever created in
...
export async function ingestEmail({ from, subject, text, messageId }) {
  ...
  const drop = async (reason) => {
    await logDrop({ reason, from, subject });
    return { ok: false, reason };
  };

  if (!sender) return drop("no_sender");
  if (!isAllowedSender(from, settings)) return drop("sender_not_allowed");
  ...
  // Two ceilings, both counted over today's email leads. The per-sender one is
  // the point: counting every sender together (as this did) let one chatty
  // broker use up the whole day's budget and silently starve the others.
  const since = todayISO() + "T00:00:00Z";
  const todaysEmail = () => admin.from("job_leads")
    .select("id", { count: "exact", head: true })
    .eq("source", "email").gte("created_at", since);

  // A failed count must not read as zero and quietly wave the cap through, so
  // it is logged and the global ceiling below still applies.
  const { count: mine, error: mineErr } = await todaysEmail().eq("parsed->>sender", sender);
  if (mineErr) console.error("[pipeline] per-sender count:", mineErr.message);
  if ((mine || 0) >= settings.maxLeadsPerSenderPerDay) return drop("rate_limited");

  const { count: all, error: allErr } = await todaysEmail();
  if (allErr) console.error("[pipeline] daily count:", allErr.message);
  if ((all || 0) >= settings.maxLeadsPerDay) return drop("day_cap");

  const body = [subject ? `Subject: ${subject}` : "", String(text || "")].filter(Boolean).join("\n\n");
  const lead = await createLeadFromText({ text: body, source: "email", sourceRef: messageId || null, sender, requireJob: true });
  // Newsletters, receipts, account notices: logged like any other drop, so the
  // board shows it and a real job misread as noise can still be pasted by hand.
  if (!lead) return drop("not_a_job");
