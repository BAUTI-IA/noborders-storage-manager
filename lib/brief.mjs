...
import { sweepHolds, calibrateDelivered } from "./leads.mjs";
...
export async function collectBriefData() {
  ...
  // Pipeline: the hold clock. sweepHolds both advances it (marks the day-2
  // reminder sent, expires whatever blew past day 7) and hands back the three
  // buckets, so the morning brief IS the alarm the workflow asked for. It runs
  // inside the cron that already exists — the Hobby plan allows no other.
  let pipeline = { recordatorio: [], vence_hoy: [], vencidos: [] };
  try {
    const sw = await sweepHolds(today);
    const lead = (l) => ({
      lead: l.broker_job_number || `#${l.id}`,
      cliente: l.customer || "sin nombre",
      precio: num(l.broker_price),
      cf: num(l.cu_ft),
      ruta: [l.origin_city, l.dest_city].filter(Boolean).join(" → "),
      decidir_antes_de: l.hold_until || null,
    });
    pipeline = {
      recordatorio: sw.remind.map(lead),
      vence_hoy: sw.due.map(lead),
      vencidos: sw.expired.map(lead),
    };
  } catch (e) {
    console.error("pipeline sweep:", e); // the module may not be installed yet
  }

  // Calibration: fill in what recently delivered jobs REALLY cost, so the cost
  // model corrects itself instead of ageing. Best-effort and quiet — a failure
  // here must never cost the team its morning brief.
  let calibracion = { completados: [], sin_datos: 0 };
  try {
    const cal = await calibrateDelivered(today);
    calibracion = { completados: cal.filled, sin_datos: cal.skipped };
  } catch (e) {
    console.error("calibration sweep:", e);
  }
...
function fallbackBrief(d) {
  return [
    `📋 Daily brief — ${d.fecha}`,
    `🚚 Today: ${d.agenda.pickups_hoy_total} pickups, ${d.agenda.entregas_hoy_total} deliveries`,
    `💰 Real collectable debt: ${money(d.cobros.deuda_real_total)} (${d.cobros.deudores_total} jobs) · uncleared old balances: ${money(d.cobros.balances_sin_depurar)}`,
    `🏬 Storage not billed: ${d.fugas_storage.jobs_en_deposito_sin_billing} jobs (~${money(d.fugas_storage.perdida_mensual_estimada)}/mo) · ${d.fugas_storage.billing_vencidos} overdue invoices`,
    `⚠️ FADD overdue: ${d.fadd.vencidos_total} · due within 48h: ${d.fadd.vencen_48h.length}`,
    `📎 Open claims: ${d.claims_activos.total} (${money(d.claims_activos.monto_reclamado)})`,
    `📥 Pipeline: ${d.pipeline.recordatorio.length} reminders, ${d.pipeline.vence_hoy.length} due today, ${d.pipeline.vencidos.length} overdue`,
    `📐 Calibration: ${d.calibracion.completados.length} delivered job(s) priced against what they really cost`,
  ].join("\n");
}
    ...
    const msg = await client.messages.create({
      ...
      messages: [{
        ...
        content: [
          ...
          "",
          "Style: ENGLISH (US business-casual), plain Telegram text (no Markdown), emojis as section markers, short lines. Under 3600 characters. Note: the data JSON below uses Spanish keys (deuda_real_total = real collectable debt, fugas = leaks, vencidos = overdue, etc.) — the output must be fully in English.",
          "Structure:",
          "1. Today's schedule: pickups/deliveries with customer and what to collect on each.",
          "2. 💰 COLLECTIONS — the star section. deuda_real_total is money genuinely collectable NOW (balances minus payments already recorded); balances_sin_depurar is old uncleared balances (data hygiene, one line only, do not mix into the debt). List top debtors with amounts, action tone (\"go collect these today\").",
          "3. 🏬 Storage billing leaks: jobs sitting in storage with no active billing, and overdue storage invoices, with the money being lost.",
          "4. ⚠️ FADD alerts: oldest overdue + the ones due within 48h.",
          "5. Claims in one line.",
          "5c. 📐 CALIBRACIÓN — `calibracion.completados` son jobs entregados a los que se les acaba de cargar el costo real (días, millas, combustible). Mencionalos en UNA línea sólo si hay: cuántos jobs y que el modelo de costos se actualizó con ellos. Los marcados `compartido:true` viajaron con otros jobs, así que el Job Calculator no aprende de ellos — no hace falta explicarlo en el brief. Si la lista está vacía, omití la sección entera.",
          "5b. 📥 PIPELINE — incoming work waiting on a decision. `pipeline.recordatorio` are leads hitting their day-2 reminder; `vence_hoy` must be decided today; `vencidos` blew past the 7-day limit and now need a call: do it ourselves or hand it to a partner carrier. Name each one with its customer, route and price. Skip the section when all three lists are empty.",
          "6. 📨 READY-TO-SEND COLLECTION MESSAGES: for the top 2-3 debtors that have entregado=true, write a short, courteous-but-firm collection message ready to forward to the client (\"Hi {first name}, this is No Borders Moving. Our records show an outstanding balance of $X for your move (job #...). ...\"), and show the client's phone next to each so the team can send it. If none qualify, skip the section.",
          "7. Close with a one-line priority for the day.",
          deltas ? `DELTAS vs ${deltas.vs_fecha} (show them inline next to each metric: debt \"↓ $X vs yesterday ✅\" if negative / \"↑ $X vs yesterday 🔺\" if positive / \"no change\" if 0; same idea for FADD, leaks and claims): ${JSON.stringify(deltas)}` : "No previous snapshot — omit day-over-day comparisons.",
          "Rules: only use the data below, never invent; amounts as $1,234; dates MM/DD (US); if a section is empty, one short positive line; job numbers verbatim.",
          "",
