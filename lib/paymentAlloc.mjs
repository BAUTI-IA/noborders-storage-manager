...
// A client can pay one job several ways at once (cash + Zelle + two money
// orders...). Each saved payments row keeps ONE method and ONE charge, so the
// method lines are poured over the charge rows in order: the first line fills
// the first charge, spills into the next, and so on. `unassigned` is the
// on-account remainder from serializeAllocLines; a line that outruns every
// charge lands there too.
// Returns [{ line, charge, amount }] — charge is null for the on-account part.
export function pourLinesOverCharges(lines, chargeRows, unassigned) {
  const charges = (chargeRows || []).map(c => ({ row: c, left: num(c.amount) }));
  if (num(unassigned) > 0.009) charges.push({ row: null, left: num(unassigned) });
  const out = []; let ci = 0;
  for (const line of lines || []) {
    let left = num(line.amount);
    while (left > 0.009 && ci < charges.length) {
      const c = charges[ci]; const take = Math.min(left, c.left);
      if (take > 0.009) { out.push({ line, charge: c.row, amount: round2(take) }); c.left = round2(c.left - take); left = round2(left - take); }
      if (c.left <= 0.009) ci++;
    }
    if (left > 0.009) out.push({ line, charge: null, amount: round2(left) });
  }
  return out;
}
