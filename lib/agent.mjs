...
import { getGuide, guideIndex, rememberTurns, searchCrmText, searchMemory } from "./retrieval.mjs";

// Sonnet 5 is near-Opus on agentic/tool work and answers much faster, which is
// what matters for a chat assistant; both are overridable per environment.
...
const T = {
  es: {
    ...
    nothingPending: "No tengo ninguna propuesta pendiente para confirmar.",
  },
  en: {
    ...
    nothingPending: "There is no pending proposal to confirm.",
  },
};
...
// Free-text search across the CRM's notes and descriptions. The sql tool can
// only find these with a lucky ilike on the exact word; this ranks by how many
// of the query's terms actually appear, and tolerates accents and plurals.
const SEARCH_TOOL = {
  name: "search",
  description: "Search the free text people typed into the CRM — claim descriptions, job and carrier notes, payment and expense comments, trip stops, broker/driver notes. Use it when the question is about something SAID rather than something counted: \"the job where the client complained about a broken mirror\", \"which expense mentions the warehouse door\". For numbers, dates, statuses or anything you can filter with a column, use sql instead.",
  input_schema: {
    type: "object",
    properties: {
      query: { type: "string", description: "Words to look for, in the language they were most likely written in. Keep it to the distinctive terms — no filler words." },
      scope: {
        type: "string",
        enum: ["crm", "memory", "both"],
        description: "\"crm\" (default) searches the notes people typed into the CRM. \"memory\" searches what THIS person told you in past conversations, beyond the recent turns you can see — use it for \"what did I tell you about…\", \"did we talk about…\". \"both\" when either could hold the answer.",
      },
    },
    required: ["query"],
  },
...
// The CRM's own user guide. It's small enough (~2k tokens per language) that
// the tool hands back the whole thing; `section` is there for the day it grows
// past the budget, when getGuide() starts returning the index instead.
const GUIDE_TOOL = {
  name: "guide",
  description: "Read the CRM's user guide: how the app is USED — screens, buttons, form fields, colour codes, step-by-step flows. Use it for \"how do I…\", \"what does X mean\", \"where do I find…\". It describes the interface, not the data: for real jobs, money or dates use sql instead.",
  input_schema: {
    type: "object",
    properties: {
      lang: { type: "string", enum: ["es", "en"], description: "Language the user is writing in." },
      section: { type: "string", description: "Optional section id (see the index in the system prompt). Omit it to get the whole guide, which is usually what you want." },
    },
  },
};
...
// Both caches hold the in-flight *promise*, not the resolved text: two requests
// hitting a cold instance at once then share one round trip instead of racing.
let schemaCache = { p: null, at: 0 };
export async function getDbSchema() {
  if (schemaCache.p && Date.now() - schemaCache.at < 10 * 60 * 1000) return schemaCache.p;
  schemaCache = {
    at: Date.now(),
    p: (async () => {
      ...
      return data.map((r) => `${r.table_name}: ${r.cols}`).join("\n");
    })(),
  };
  schemaCache.p.catch(() => { schemaCache = { p: null, at: 0 }; }); // don't cache a failure
  return schemaCache.p;
}

const recipeCatalog = () => Object.entries(RECIPES).map(([n, r]) => `- ${n} ${r.doc || ""}`).join("\n");

// Small, slow-changing lists the agent would otherwise have to look up with a
// sql round trip on almost every request (each round trip is a full model call,
// i.e. seconds of latency). Preloading them makes name→id resolution instant.
let refCache = { p: null, at: 0 };
export async function getReferenceData() {
  if (refCache.p && Date.now() - refCache.at < 5 * 60 * 1000) return refCache.p;
  refCache = {
    at: Date.now(),
    p: (async () => {
      // Never let a missing column break the agent: each list degrades to empty.
      const safe = async (fn) => { try { const { data } = await fn(); return data || []; } catch { return []; } };
      const [brokers, drivers, trucks, trips, storages] = await Promise.all([
        safe(() => admin.from("brokers").select("id, name").is("deleted_at", null).limit(200)),
        safe(() => admin.from("drivers").select("id, name, active").is("deleted_at", null).limit(200)),
        safe(() => admin.from("trucks").select("id, name, plate, capacity_cf, active").is("deleted_at", null).limit(200)),
        safe(() => admin.from("trips").select("id, trip_number, status, departure_date").is("deleted_at", null).in("status", ["loading", "in_transit"]).limit(60)),
        safe(() => admin.from("storages").select("id, unit, warehouse").is("deleted_at", null).limit(200)),
      ]);
      const list = (rows, fmt) => rows.map(fmt).join(" · ") || "(none)";
      return [
        `BROKERS: ${list(brokers, (b) => `${b.id}=${b.name}`)}`,
        `DRIVERS: ${list(drivers, (d) => `${d.id}=${d.name}${d.active === false ? " (inactive)" : ""}`)}`,
        `TRUCKS: ${list(trucks, (t) => `${t.id}=${t.name || t.plate}${t.capacity_cf ? ` (${t.capacity_cf}cf)` : ""}${t.active === false ? " (inactive)" : ""}`)}`,
        `OPEN TRIPS: ${list(trips, (t) => `${t.id}=${t.trip_number || "(no #)"} [${t.status}]${t.departure_date ? ` departs ${t.departure_date}` : ""}`)}`,
        `STORAGES: ${list(storages, (s) => `${s.id}=${s.unit || "(no #)"}${s.warehouse ? `@${s.warehouse}` : ""}`)}`,
      ].join("\n");
    })(),
  };
  refCache.p.catch(() => { refCache = { p: null, at: 0 }; });
  return refCache.p;
}

// Warms both caches without blocking: called as soon as a message arrives so the
// two round trips overlap with loading the conversation instead of following it.
export function warmCaches() {
  getDbSchema().catch(() => {});
  getReferenceData().catch(() => {});
}

export function actorLine(actor) {
...
async function buildSystemPrompt({ actor, canWrite }) {
  ...
  const [schema, directory] = await Promise.all([getDbSchema(), getReferenceData()]);
  return [
    ...
    schema,
    "",
    "CRM DIRECTORY (ids ready to use — do NOT run sql to resolve these names):",
    directory,
    "",
    "FREE TEXT — notes, claim descriptions and comments are NOT in the directory above and are hard to reach with sql. Use the `search` tool when the question is about something someone WROTE. RECENT CONVERSATION below is only the last few turns: if they refer to something older (\"lo que te dije la semana pasada\"), search with scope \"memory\" instead of saying you don't remember.",
    "",
    "CRM USER GUIDE — call the `guide` tool (passing the user's language) to read how the app is used. Sections:",
    guideIndex("es"),
    "",
    ...
    "- Live load = the trips map: a truck's current position lives on trucks.last_lat/last_lng/last_location/last_status. When someone tells you where a truck is (\"el 3 está saliendo de Ocala\"), use set_truck_location — it geocodes the address itself.",
    "- Deletes are always recoverable (soft delete). Never propose one unless the user clearly asked for it.",
    "",
    "HOW TO WORK:",
    "0. You write across the WHOLE CRM: trips and live load, payments, expenses, billing, jobs, claims, fleet, brokers, drivers. Never reply that you \"can't do that in the CRM\" — either propose the plan, or say exactly what is missing (a piece of data, or a permission you don't have).",
    "1. Be fast: brokers, drivers, trucks, open trips and storages are already listed above — use those ids directly, no sql needed. Only query when you genuinely need job data or numbers you don't have. Never invent an id or a fact.",
    "2. If the request is a question about DATA, just answer it — no plan. If it asks how the CRM is USED (how to load an extra, what a colour means, where a button is), call `guide` and answer from it — never guess the UI.",
...
// ── The loop ─────────────────────────────────────────────────────────────────
async function runAgent({ text, history, attachments, draft, actor, canWrite, onEvent, convoKey }) {
  ...
  const tools = canWrite ? [SQL_TOOL, SEARCH_TOOL, GUIDE_TOOL, PLAN_TOOL] : [SQL_TOOL, SEARCH_TOOL, GUIDE_TOOL];

  const emit = (ev) => { try { onEvent?.(ev); } catch { /* the channel is best-effort */ } };
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    let msg;
    if (onEvent) {
      // Stream so the user sees the answer being written instead of staring at
      // a spinner for the whole round trip. The tool loop still runs on the
      // finished message.
    ...
    // Whatever text came before the tool call was a preamble ("let me check…");
    // drop it from the live bubble so it isn't duplicated by the real answer.
    emit({ type: "reset" });
    ...
    for (const tu of toolUses) {
      if (tu.name === "sql") {
        emit({ type: "status", text: "🔎 Consultando la base…" });
      ...
      if (tu.name === "search") {
        const scope = tu.input?.scope === "memory" || tu.input?.scope === "both" ? tu.input.scope : "crm";
        emit({ type: "status", text: scope === "memory" ? "🧠 Buscando en conversaciones anteriores…" : "🔎 Buscando en notas y comentarios…" });
        let content;
        try {
          const [crm, mem] = await Promise.all([
            scope === "memory" ? null : searchCrmText({ query: tu.input?.query, profile: actor.profile }),
            scope === "crm" ? null : searchMemory({ query: tu.input?.query, convoKey, userEmail: actor.userEmail }),
          ]);
          content = [crm, mem].filter(Boolean).join("\n\n");
        } catch (e) { content = `ERROR: ${e.message}`; }
        results.push({ type: "tool_result", tool_use_id: tu.id, content, is_error: content.startsWith("ERROR:") });
        continue;
      }
      if (tu.name === "guide") {
        emit({ type: "status", text: "📚 Leyendo la guía del CRM…" });
        results.push({
          type: "tool_result",
          tool_use_id: tu.id,
          content: getGuide({ lang: tu.input?.lang, section: tu.input?.section }),
        });
        continue;
      }
      // stage_plan
      emit({ type: "status", text: "📝 Armando el plan…" });
...
// onEvent (optional) receives { type:"delta"|"status"|"reset", … } while the
// agent works — the in-app chat streams these so the reply appears as it's
// written. Channels without streaming (Telegram, WhatsApp) just omit it.
// opts (optional), used by the voice channel (lib/voice.mjs):
//   readOnly  — answer the question but never stage a write.
//   decision  — "confirm" | "cancel" the pending plan explicitly, instead of
//               guessing it from the wording. A spoken "sí" reaches us as
//               whatever the transcriber heard, so voice decides in the tool
//               call and doesn't leave it to a regex.
//   detail    — resolve to { reply, state, pending } instead of just the text.
export async function handleIncoming(convoKey, text, attachments = [], actor = {}, onEvent = null, opts = {}) {
  const ctx = { profile: actor.profile || null, userEmail: actor.userEmail || actor.profile?.email || null };
  const canWrite = !!ctx.profile && writesEnabled() && !opts.readOnly;
  try {
    warmCaches(); // overlap the schema/directory fetch with the conversation read
    ...
    if (opts.decision && !pending) {
      out = { reply: tr(convo?.pending_action?.lang).nothingPending };
    } else if (pending && !attachments.length && (opts.decision === "confirm" || (!opts.decision && isConfirm(text)))) {
      const t = tr(pending.lang);
      if (stale) out = { reply: t.expired };
      else {
        try { onEvent?.({ type: "status", text: "⚙️ Ejecutando…" }); } catch { /* best-effort */ }
        const res = await executePlan(pending.plan, ctx);
        out = { reply: res.ok
          ? `${t.done}\n${res.done.map((d) => `• ${d}`).join("\n")}`
          : `${t.partial}\n${res.done.map((d) => `• ${d}`).join("\n")}\n\n${res.error}` };
      }
    } else if (pending && !attachments.length && (opts.decision === "cancel" || (!opts.decision && isCancel(text)))) {
      ...
      out = await runAgent({ text, history, attachments, draft: stale ? null : pending, actor: ctx, canWrite, onEvent, convoKey });
    }
    ...
    await Promise.all([
      admin.from("wa_conversations").upsert({
        phone: convoKey,
      ...
      // newHistory se recorta a los últimos turnos; esto los guarda enteros para
      // que el agente pueda recordar más allá de esa ventana.
      rememberTurns(convoKey, ctx.userEmail, [
        { role: "user", text: userLine },
        { role: "assistant", text: out.reply },
      ]),
    ]);

    const reply = String(out.reply).slice(0, REPLY_MAX + 600);
    return opts.detail ? { reply, state: out.state || "idle", pending: out.pending || null } : reply;
  } catch (e) {
    console.error("agent:", e);
    const reply = "⚠️ Hubo un error procesando el mensaje / Something went wrong. Try again in a moment.";
    return opts.detail ? { reply, state: "idle", pending: null } : reply;
  }
}
