...
export async function getReferenceData() {
  ...
  refCache = {
    ...
    p: (async () => {
      ...
      const list = (rows, fmt) => rows.map(fmt).join(" · ") || "(ninguno)";
      return [
        `BROKERS: ${list(brokers, (b) => `${b.id}=${b.name}`)}`,
        `DRIVERS: ${list(drivers, (d) => `${d.id}=${d.name}${d.active === false ? " (inactivo)" : ""}`)}`,
        `TRUCKS: ${list(trucks, (t) => `${t.id}=${t.name || t.plate}${t.capacity_cf ? ` (${t.capacity_cf}cf)` : ""}${t.active === false ? " (inactivo)" : ""}`)}`,
        `TRIPS ABIERTOS: ${list(trips, (t) => `${t.id}=${t.trip_number || "s/n"} [${t.status}]${t.departure_date ? ` sale ${t.departure_date}` : ""}`)}`,
        `STORAGES: ${list(storages, (s) => `${s.id}=${s.unit || "s/n"}${s.warehouse ? `@${s.warehouse}` : ""}`)}`,
      ].join("\n");
    })(),
  };
