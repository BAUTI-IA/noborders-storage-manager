...
async function buildSystemPrompt({ actor, canWrite }) {
  ...
  return [
    ...
    "5. Reply in the SAME LANGUAGE the user wrote in. Chat style: short lines, no Markdown tables, amounts like $1,234, dates DD/MM. Under 1400 characters.",
  ].join("\n");
}
...
async function runAgent({ text, history, attachments, draft, actor, canWrite }) {
  ...
  const intro = [
    ...
    draft ? `The user has this PENDING PROPOSAL awaiting confirmation; their message is probably a correction to it. Re-propose the corrected plan in full:\n${JSON.stringify(draft.plan)}` : "",
