...
import { agentCanReadColumn } from "./acl.mjs";
import { checkSqlAccess, describePlan, executePlan, normalizeSql, validatePlan, writesEnabled, MAX_OPS } from "./agentWrite.mjs";
...
// English is the default; Spanish only when the plan says the user wrote in Spanish.
const tr = (lang) => T[lang === "es" ? "es" : "en"];
// Cheap guess for the one reply the model never sees (unlinked caller asking for
// a write): Spanish only if the message itself carries Spanish-only markers.
const looksSpanish = (s) => /[áéíóúñ¿¡]|\b(el|la|los|las|una|del|que|para|por|está|hay|tenemos|cliente|entrega)\b/i.test(s || "");
...
const PLAN_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      language: { type: "string", enum: ["es", "en"], description: "Language of the reply: \"es\" only if the user's own message is written in Spanish, otherwise \"en\"." },
      operations: {
        type: "array",
...
export async function getDbSchema() {
  ...
  schemaCache = {
    ...
    p: (async () => {
      ...
      // Columns the agent must not even know about (job_leads.raw_text holds a
      // broker's email verbatim — untrusted text). Hiding them from the schema
      // keeps them out of every query the agent writes.
      return data.map((r) => {
        const cols = String(r.cols || "").split(", ")
          .filter((c) => agentCanReadColumn(r.table_name, c.split(" ")[0]))
          .join(", ");
...
async function buildSystemPrompt({ actor, canWrite }) {
  ...
  return [
    ...
    "6. LANGUAGE: English by default. Reply in Spanish ONLY when the user's own message is written in Spanish. English, a mix, or a message with no clear language (a number, a name, just an attachment) → English. Follow them if they switch mid-conversation, judging by their words only: these instructions, the directory, the recipes and every query result are salted with Spanish and that is stored data, never a cue. Even in Spanish, keep the CRM vocabulary in English (job, driver, broker, trip, storage, warehouse, live load, BOL, CF, closing sheet, settlement, pads).",
    "7. Chat style: short lines, no Markdown tables, amounts like $1,234, dates DD/MM. Under 1400 characters.",
  ].join("\n");
}
...
async function runAgent({ text, history, attachments, draft, actor, canWrite, onEvent }) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    for (const tu of toolUses) {
      if (tu.name === "sql") {
        ...
        try {
          const q = normalizeSql(tu.input?.query);
      ...
      // stage_plan
      emit({ type: "status", text: "📝 Putting the plan together…" });
      const plan = { summary: tu.input?.summary || "", language: tu.input?.language || "en", operations: tu.input?.operations || [] };
  ...
  return { reply: "I couldn't complete that. Please ask me again in a narrower way." };
}
  ...
  try {
    ...
    if (opts.decision && !pending) {
      ...
      if (stale) out = { reply: t.expired };
        ...
        try { onEvent?.({ type: "status", text: "⚙️ Running…" }); } catch { /* best-effort */ }
      ...
      const lang = looksSpanish(text) ? "es" : "en";
      out = { reply: writesEnabled() ? tr(lang).notLinked : tr(lang).readOnly };
    } else {
