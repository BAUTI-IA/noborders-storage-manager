...
const YES_RE = /^\s*(ok(ay)?|s[ií]|dale|confirmo|confirmar|listo|va|yes|yep|sure|confirm(ed)?|go( ahead)?|correct)\b/i;
const NO_RE = /^\s*(no(pe)?|cancel(a|ar|o)?|nada|olvidalo|olv[ií]dalo|forget it|nevermind|never mind|stop)\b/i;
...
async function commitPending(pending) {
  ...
  if (pending.type === "create_job") {
    ...
    return `${t.created}\n${summarize(pending.data, pending.lang)}`;
  }
...
async function handleCreate(intent) {
  ...
  return {
    ...
    pending: { type: "create_job", data, lang },
    reply: `${t.willCreate}\n${summarize(data, lang)}\n\n${t.confirm}`,
  };
}
...
async function handleUpdate(intent) {
  ...
  return {
    state: "pending_confirmation",
    pending: { type: "update_job", job_ids: rows.map((r) => r.id), job_number: jobNumber, data: patch, lang },
    reply: `${t.willUpdate(jobNumber, customers[0])}\n${summarize(patch, lang)}\n\n${t.confirm}`,
  };
}
