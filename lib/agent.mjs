...
import { getGuide, guideIndex, searchCrmText } from "./retrieval.mjs";
...
async function buildSystemPrompt({ actor, canWrite }) {
  ...
  return [
    ...
    directory,
    "",
    "FREE TEXT — notes, claim descriptions and comments are NOT in the directory above and are hard to reach with sql. Use the `search` tool when the question is about something someone WROTE.",
    "",
...
// ── The loop ─────────────────────────────────────────────────────────────────
async function runAgent({ text, history, attachments, draft, actor, canWrite, onEvent }) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    for (const tu of toolUses) {
      ...
      if (tu.name === "search") {
        emit({ type: "status", text: "🔎 Buscando en notas y comentarios…" });
        let content;
        try {
          content = await searchCrmText({ query: tu.input?.query, profile: actor.profile });
...
export async function handleIncoming(convoKey, text, attachments = [], actor = {}, onEvent = null, opts = {}) {
  ...
  try {
    ...
    if (opts.decision && !pending) {
      ...
      out = await runAgent({ text, history, attachments, draft: stale ? null : pending, actor: ctx, canWrite, onEvent });
    }
