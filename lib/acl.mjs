...
export const TABLE_ACL = {
  storages:             { view: ["storage", "jobs", "dispatching", "calendario", "calendario_entregas", "billing", "apar"], owner: "storage" },
  storage_jobs:         { view: ["jobs", "dispatching", "calendario", "calendario_entregas", "storage", "billing", "brokers", "settlements", "clientes", "apar"], owner: "jobs" },
  job_events:           { view: ["dispatching", "jobs"], owner: "dispatching" },
  brokers:              { view: ["brokers", "apar"], owner: "brokers" },
  storage_billing:      { view: ["billing", "apar"], owner: "billing" },
  closing_sheets:       { view: ["settlements", "apar"], owner: "settlements" },
  job_extras:           { view: ["extras", "payments", "analytics", "apar"], owner: "extras" },
  employees:            { view: ["extras", "settings", "drivers"], owner: "settings" },
  drivers:              { view: ["drivers", "dispatching", "jobs", "apar"], owner: "drivers" },
  ...
  payments:             { view: ["payments", "analytics", "apar"], owner: "payments" },
  ...
  // Expenses module.
  expenses:             { view: ["expenses", "drivers", "analytics", "apar"], owner: "expenses" },
  driver_work_days:     { view: ["expenses", "drivers", "analytics", "apar"], owner: "expenses" },
  driver_adjustments:   { view: ["expenses", "drivers", "analytics", "apar"], owner: "expenses" },
  ...
  bank_categories:      { view: ["bancos", "apar"], owner: "bancos" },
  bank_import_batches:  { view: ["bancos"], owner: "bancos" },
  bank_transactions:    { view: ["bancos", "analytics", "apar"], owner: "bancos" },
  // AP / AR module. Everything it shows on the receivable side is derived from
  // tables owned elsewhere (hence the "apar" reads added above and below);
  // ap_bills is the one payable it owns outright.
  ap_bills:             { view: ["apar", "analytics"], owner: "apar" },
  // Pipeline module. A lead is an opportunity that has not been decided yet.
  job_leads:            { view: ["pipeline", "jobs", "analytics"], owner: "pipeline" },
};
...
export const AGENT_DENY_TABLES = new Set([
  ...
  "zip_distances", "zip_geo", "geo_cache", "job_calc_settings", "suggestion_votes",
  "pipeline_settings",
]);
...
export const SOFT_DELETE_TABLES = new Set([
  ...
  "trip_stops", "job_events", "payment_accounts", "ap_bills", "job_leads",
]);
...
// Columns the agent must never pull into its own context. job_leads.raw_text is
// a broker's email verbatim — untrusted text that could try to steer the agent
// if it were read back as context — so the agent sees every other column but
// not that one. The UI, which renders it as data, is unaffected.
export const AGENT_DENY_COLUMNS = { job_leads: new Set(["raw_text"]) };
export const agentCanReadColumn = (table, column) => !AGENT_DENY_COLUMNS[table]?.has(column);
