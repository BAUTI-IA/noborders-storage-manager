...
// Live load: same geocoder the map's "set location" modal uses (api/geocode.mjs),
// called directly so the recipe doesn't depend on the app's own URL.
async function geocode(address) {
  const url = "https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=us,ca&q=" + encodeURIComponent(address);
  const r = await fetch(url, { headers: { "User-Agent": "NoBordersMovingCRM/1.0 (live-load map geocoding)", Accept: "application/json" } });
  if (!r.ok) throw new Error(`el geocoder respondió ${r.status}`);
  const data = await r.json();
  if (!Array.isArray(data) || !data.length) throw new Error(`no encontré la dirección "${address}"`);
...
export const RECIPES = {
  ...
  // Live load: where a truck is right now. Mirrors the map's "set location"
  // modal (App.jsx saveLoc) — the address is geocoded so the truck actually
  // shows up on the map, not just in the list.
  set_truck_location: {
    perms: [["trucks", "update"]],
    doc: "{ truck_id, address?, lat?, lng?, label?, status?: 'moving'|'stopped'|'unknown' } — posición del camión en el mapa de live load; con `address` la geocodifica sola",
    describe: (a) => `Ubicar el camión #${a.truck_id} en ${a.label || a.address || `${a.lat}, ${a.lng}`}${a.status ? ` (${a.status})` : ""}`,
    validate: async (a) => {
      const errs = [];
      const { data: t } = await admin.from("trucks").select("id").eq("id", a.truck_id).is("deleted_at", null).maybeSingle();
      if (!t) errs.push(`no encuentro el camión #${a.truck_id}`);
      if (!a.address && !(Number.isFinite(Number(a.lat)) && Number.isFinite(Number(a.lng))))
        errs.push("necesito una dirección (address) o las coordenadas (lat/lng)");
      if (a.status && !["moving", "stopped", "unknown"].includes(a.status))
        errs.push(`estado inválido: "${a.status}" (moving | stopped | unknown)`);
      return errs;
    },
    run: async (a, ctx) => {
      let { lat, lng } = { lat: Number(a.lat), lng: Number(a.lng) };
      let label = a.label || a.address || null;
      if (!(Number.isFinite(lat) && Number.isFinite(lng))) {
        const g = await geocode(a.address);
        lat = g.lat; lng = g.lng;
        label = a.label || g.label;
      }
      return {
        entries: await execUpdate("trucks", [a.truck_id], {
          last_lat: lat, last_lng: lng, last_location: label,
          last_status: a.status || "stopped", last_location_at: new Date().toISOString(),
        }, ctx),
        summary: `Camión #${a.truck_id} ubicado en ${label}`,
      };
    },
  },
  ...
  // Un trabajo que llega por WhatsApp/Telegram entra al Pipeline como LEAD, no
  // como job: todavía no se decidió tomarlo. La decisión (accept / hold /
  // reject / offer) la toma una persona en el tablero.
  create_lead: {
    perms: [["job_leads", "insert"]],
    doc: "{ customer?, broker_id?, broker_job_number?, origin_zip?, origin_city?, origin_state?, dest_zip?, dest_city?, dest_state?, cu_ft?, broker_price?, fadd?, pickup_date_from?, pickup_date_to?, job_type?, notes? }",
    describe: (a) => {
      const ruta = [a.origin_city || a.origin_zip, a.dest_city || a.dest_zip].filter(Boolean).join(" → ");
      const plata = a.broker_price ? ` por ${"$" + num(a.broker_price).toLocaleString("en-US")}` : "";
      const cf = a.cu_ft ? `, ${num(a.cu_ft)} cf` : "";
      return `Cargar un lead en el Pipeline${a.customer ? ` de ${a.customer}` : ""}${ruta ? ` (${ruta})` : ""}${cf}${plata}`;
    },
    validate: async (a) => {
      const errs = [];
      const zip = (z) => !z || /^\d{5}$/.test(String(z).trim());
      if (!zip(a.origin_zip)) errs.push("origin_zip tiene que ser un ZIP de 5 dígitos");
      if (!zip(a.dest_zip)) errs.push("dest_zip tiene que ser un ZIP de 5 dígitos");
      if (a.job_type && !["full", "direct", "broker_delivery"].includes(a.job_type)) {
        errs.push(`job_type inválido: "${a.job_type}" (full, direct o broker_delivery)`);
      }
      if (a.broker_id != null) {
        const { data: b } = await admin.from("brokers").select("id").eq("id", a.broker_id).is("deleted_at", null).maybeSingle();
        if (!b) errs.push(`no encuentro el broker #${a.broker_id}`);
      }
      if (!a.customer && !a.broker_job_number && !a.origin_zip && !a.dest_zip) {
        errs.push("hace falta al menos el cliente, el número del broker o una de las dos puntas");
      }
      return errs;
    },
    run: async (a, ctx) => {
      const d = (v) => (/^\d{4}-\d{2}-\d{2}$/.test(String(v || "")) ? String(v) : null);
      const z = (v) => (v ? String(v).trim().slice(0, 5) : null);
      const row = {
        source: "whatsapp",
        status: "new",             // nunca nace decidido
        customer: a.customer || null,
        broker_id: a.broker_id ?? null,
        broker_job_number: a.broker_job_number || null,
        origin_zip: z(a.origin_zip), origin_city: a.origin_city || null, origin_state: a.origin_state || null,
        dest_zip: z(a.dest_zip), dest_city: a.dest_city || null, dest_state: a.dest_state || null,
        cu_ft: a.cu_ft == null ? null : num(a.cu_ft),
        broker_price: a.broker_price == null ? null : num(a.broker_price),
        fadd: d(a.fadd), pickup_date_from: d(a.pickup_date_from), pickup_date_to: d(a.pickup_date_to),
        job_type: a.job_type || null,
        notes: a.notes || null,
      };
      const entries = await execInsert("job_leads", row, ctx);
      const ruta = [row.origin_city || row.origin_zip, row.dest_city || row.dest_zip].filter(Boolean).join(" → ");
      return {
        entries,
        summary: `Lead cargado en el Pipeline${row.customer ? ` (${row.customer}` : ""}${ruta ? `${row.customer ? ", " : " ("}${ruta}` : ""}${row.customer || ruta ? ")" : ""}. Falta decidirlo en el tablero.`,
      };
    },
  },
};
