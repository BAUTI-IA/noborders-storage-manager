-- Pipeline — migración única (job_leads + pipeline_settings).
--
-- Dos formas de correrla, elegí una:
--   A) Supabase → SQL Editor → pegar todo esto → Run.
--   B) SUPABASE_ACCESS_TOKEN=sbp_xxx node scripts/setup-pipeline.mjs
--
-- Es idempotente: correrla dos veces no rompe nada.
-- Este archivo es el MISMO SQL que scripts/setup-pipeline.mjs — si tocás uno,
-- tocá el otro.
