...
const I18N_ES = {
  ...
  "Codriver": "Co-driver",
  "Crew — codriver / helpers": "Equipo — co-driver / helpers",
  "+ Add crew member": "+ Agregar acompañante",
  "Max 3 people per truck (foreman + 2 crew).": "Máximo 3 personas por camión (foreman + 2 acompañantes).",
  "Run the setup SQL to save the trip crew.": "Corré el SQL de configuración para guardar el equipo del trip.",
  "— Person —": "— Persona —",
  "Remove from crew": "Quitar del equipo",
  ...
  "Foreman driver": "Driver principal (foreman)",
...
const EMPTY_TRIP = { trip_number:"", truck_id:"", driver_id:"", departure_date:"", status:"loading", notes:"", job_keys:[], crew:[] };
// Crew on the truck besides the foreman driver (1-3 people per truck total).
// Stored on trips.crew as jsonb [{driver_id, role}] for traceability + payroll.
const TRIP_CREW_ROLES = [ { v:"codriver", l:"Codriver" }, { v:"helper", l:"Helper" } ];
const TRIP_MAX_CREW = 2; // foreman + 2 = 3 people per truck
const crewRoleLabel = (r) => (TRIP_CREW_ROLES.find(x => x.v === r) || {}).l || "Helper";
const tripCrew = (t) => (Array.isArray(t?.crew) ? t.crew : []).filter(m => m && m.driver_id);
const TRIP_STATUS = {
...
-- Crew on the truck: codriver / helpers besides the foreman driver (driver_id).
alter table public.trips add column if not exists crew jsonb default '[]'::jsonb;
...
function tripManifestText(trip, truckName, driverName, jobsIn, totalCf, occPct, totalBol, crewNames) {
  const lines = [
    `TRIP #${trip.trip_number || "-"} — ${truckName || "-"}`,
    `Driver (foreman): ${driverName || "-"} | Departure: ${trip.departure_date || "-"}`,
  ];
  if (crewNames && crewNames.length) lines.push(`Crew: ${crewNames.join(", ")}`);
  lines.push(
...
export default function App() {
  ...
  const [tripCrewMissing, setTripCrewMissing] = useState(false); // trips.crew column not yet in DB
  ...
  // Probe / auto-migrate the trip crew column (codriver / helpers per trip).
  useEffect(() => {
    if (!session || tripsMissing) return;
    let cancelled = false;
    (async () => {
      const { error } = await supabase.from("trips").select("crew").limit(1);
      if (cancelled) return;
      if (!error) { setTripCrewMissing(false); return; }
      let created = false;
      for (const fn of ["exec_sql", "exec", "execute_sql"]) {
        const { error: rpcErr } = await supabase.rpc(fn, { sql: "alter table public.trips add column if not exists crew jsonb default '[]'::jsonb;" });
        if (!rpcErr) { created = true; break; }
      }
      if (cancelled) return;
      setTripCrewMissing(!created);
      if (created) loadTrips();
    })();
    return () => { cancelled = true; };
  ...
  // "Name (Codriver)" / "Name (Helper)" for everyone on the truck besides the foreman.
  const tripCrewNames = (t) => tripCrew(t).map(m => `${driverById[m.driver_id]?.name || "?"} (${crewRoleLabel(m.role)})`);
  ...
  function openEditTrip(t) {
    ...
    setTripForm({ trip_number:t.trip_number||"", truck_id:t.truck_id||"", driver_id:t.driver_id||"", departure_date:t.departure_date||"", status:t.status||"loading", notes:t.notes||"", job_keys: assigned, crew: tripCrew(t).map(m => ({ driver_id: m.driver_id, role: m.role === "codriver" ? "codriver" : "helper" })) });
    setTripJobSearch(""); setShowTripModal(true);
  }
  // ── AI trip suggestions ──────────────────────────────────────────────
  ...
  function applyTripSuggestion(s, kind) {
    ...
    if (kind === "addition") {
      ...
      setTripForm({
        ...
        crew: tripCrew(t).map(m => ({ driver_id: m.driver_id, role: m.role === "codriver" ? "codriver" : "helper" })),
      });
    } else {
      setEditingTripId(null);
      setTripForm({
  ...
  // Crew rows on the trip form (codriver / helpers besides the foreman driver).
  function tripAddCrew() {
    setTripForm(f => (f.crew || []).length >= TRIP_MAX_CREW ? f : { ...f, crew: [...(f.crew || []), { driver_id:"", role:"helper" }] });
  }
  function tripPatchCrew(idx, fields) {
    setTripForm(f => ({ ...f, crew: f.crew.map((m, i) => i === idx ? { ...m, ...fields } : m) }));
  }
  function tripRemoveCrew(idx) {
    setTripForm(f => ({ ...f, crew: f.crew.filter((_, i) => i !== idx) }));
  }
  ...
  async function saveTrip() {
    ...
    const payload = {
      ...
      // Crew (codriver / helpers) — skipped if the column isn't in the DB yet.
      ...(tripCrewMissing ? {} : { crew: (tripForm.crew || []).filter(m => m.driver_id).map(m => ({ driver_id: Number(m.driver_id), role: m.role === "codriver" ? "codriver" : "helper" })) }),
    };
                ...
                <tbody>
                  {dispatchGroups.length === 0 ? (
                    ...
                    const waHref = (gTrip && TRIP_ACTIVE(gTrip.status))
                      ? (() => { const tc = tripCalc(gTrip); return tripManifestLink(gTrip, truckById[gTrip.truck_id]?.name, driverById[gTrip.driver_id]?.name, tc.jobsIn, tc.loadedCf, tc.occPct, tc.totalBol, tripCrewNames(gTrip)); })()
      ...
      {page === "trips" && (() => {
        ...
        return (
          <>
            ...
            {tripsView === "live" ? (() => {
              // Everyone currently on each truck (from its active trip), for the side list:
              // foreman driver + codriver / helpers, so dispatch sees the whole crew.
              const driverByTruck = {};
              for (const tp of trips) {
                if (!TRIP_ACTIVE(tp.status) || !tp.truck_id) continue;
                const names = [driverById[tp.driver_id]?.name, ...tripCrewNames(tp)].filter(Boolean);
                if (names.length) driverByTruck[tp.truck_id] = names.join(" · ");
              }
                    ...
                    <div key={t.id} style={{ background:"#fff", borderRadius:12, border:"1px solid #efefef", padding:"16px 18px" }}>
                      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10, marginBottom:10 }}>
                        <div>
                          ...
                          <div style={{ fontSize:12, color:"#888", marginTop:2 }}>🚛 {truck?.name || "no truck"}{driverNm ? ` · 🧑‍✈️ ${driverNm}${crewNms.length ? " (Foreman)" : ""}` : ""}{crewNms.length ? ` · 👷 ${crewNms.join(" · ")}` : ""}{t.departure_date ? ` · ${t.departure_date}` : ""}</div>
                        </div>
                      ...
                      <div style={{ display:"flex", gap:8, marginTop:10 }}>
                        <a href={tripManifestLink(t, truck?.name, driverNm, c.jobsIn, c.loadedCf, c.occPct, c.totalBol, crewNms)} target="_blank" rel="noreferrer" style={{ textDecoration:"none", flex:1 }}><Btn primary style={{ width:"100%", justifyContent:"center" }}>💬 Send manifest to driver</Btn></a>
                    ...
                    <tbody>
                      {shown.map(t => { const c = tripCalc(t); return (
                        <tr key={t.id} style={{ borderBottom:"1px solid #fafafa" }}>
                          ...
                          <td style={{ padding:"12px" }}>
                            {(() => { const crewNms = tripCrewNames(t); return (<>
                              <div>{driverById[t.driver_id]?.name || "—"}{crewNms.length ? <span style={{ fontSize:10, color:"#aaa" }}> (Foreman)</span> : null}</div>
                              {crewNms.length > 0 && <div style={{ fontSize:11, color:"#888", marginTop:2 }}>👷 {crewNms.join(" · ")}</div>}
                            </>); })()}
                          </td>
        ...
        return (
          <Modal title={`Trip ${t.trip_number || "#"+t.id}`} onClose={() => { setTripDetailId(null); setTripAction(null); setStorageDropJob(null); setTripWaLink(null); }}
            ...
            <div style={{ fontSize:12.5, color:"#888", marginTop:-4, marginBottom:10 }}>
              <TripBadge status={t.status} /> &nbsp;🚛 {truck?.name || "no truck"}{driverNm ? ` · 🧑‍✈️ ${driverNm}${crewNms.length ? " (Foreman)" : ""}` : ""}{crewNms.length ? ` · 👷 ${crewNms.join(" · ")}` : ""}{t.departure_date ? ` · ${t.departure_date}` : ""}
            </div>
            ...
            <a href={tripManifestLink(t, truck?.name, driverNm, c.jobsIn, c.loadedCf, c.occPct, c.totalBol, crewNms)} target="_blank" rel="noreferrer" style={{ textDecoration:"none", display:"block", marginTop:10 }}><Btn style={{ width:"100%", justifyContent:"center" }}>💬 Send manifest to driver</Btn></a>
      ...
      {showTripModal && (() => {
        ...
        <Modal title={editingTripId ? "Edit trip" : "New trip"} onClose={() => setShowTripModal(false)}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            ...
            <Field label="Crew — codriver / helpers" full>
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {(tripForm.crew || []).map((m, i) => {
                  const takenIds = new Set([String(tripForm.driver_id || ""), ...tripForm.crew.filter((_, x) => x !== i).map(c => String(c.driver_id || ""))]);
                  return (
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <span style={{ fontSize:11, color:"#aaa", width:16, textAlign:"right", flexShrink:0 }}>{i + 2}.</span>
                      <select style={{ ...inp, flex:1 }} value={String(m.driver_id || "")} onChange={e => tripPatchCrew(i, { driver_id: e.target.value })}>
                        <option value="">— Person —</option>
                        {driversList.filter(d => !takenIds.has(String(d.id)) || String(d.id) === String(m.driver_id || "")).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                      </select>
                      <select style={{ ...inp, width:120, flexShrink:0 }} value={m.role === "codriver" ? "codriver" : "helper"} onChange={e => tripPatchCrew(i, { role: e.target.value })}>
                        {TRIP_CREW_ROLES.map(r => <option key={r.v} value={r.v}>{r.l}</option>)}
                      </select>
                      <button onClick={() => tripRemoveCrew(i)} title="Remove from crew" style={{ border:"none", background:"none", cursor:"pointer", color:"#E24B4A", fontSize:16, lineHeight:1, flexShrink:0 }}>×</button>
                    </div>
                  );
                })}
                {(tripForm.crew || []).length < TRIP_MAX_CREW ? (
                  <button onClick={tripAddCrew} style={{ alignSelf:"flex-start", border:"1px dashed #ccc", background:"#fff", borderRadius:7, padding:"5px 12px", cursor:"pointer", fontSize:12, color:"#555" }}>+ Add crew member</button>
                ) : (
                  <span style={{ fontSize:11, color:"#aaa" }}>Max 3 people per truck (foreman + 2 crew).</span>
                )}
                {tripCrewMissing && <span style={{ fontSize:11.5, color:"#854F0B" }}>⚠️ Run the setup SQL to save the trip crew.</span>}
              </div>
            </Field>
