...
function hmacOk(raw, header) {
  ...
  if (!key) return false; // not configured → nothing can be verified: refuse (never fail open)
...
export default async function handler(req, res) {
  ...
  // The HMAC is the only thing standing between DocuSign and anyone else who
  // finds the URL: with no secret configured the webhook refuses everything.
  if (!process.env.DOCUSIGN_CONNECT_HMAC) { res.status(503).json({ error: "server not configured: DOCUSIGN_CONNECT_HMAC" }); return; }
